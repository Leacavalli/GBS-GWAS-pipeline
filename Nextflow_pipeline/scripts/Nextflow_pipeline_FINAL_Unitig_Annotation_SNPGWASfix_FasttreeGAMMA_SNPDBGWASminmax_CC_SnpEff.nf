...
process FILTER_VCFs {
  ...
  script:
  """
  bcftools norm -a --atom-overlaps . ${params.path_nextflow_dir}/4.SNIPPY/${base}/snps.vcf.gz | bcftools view --min-af 0.95 --types snps -o ${params.path_nextflow_dir}/4.SNIPPY/${base}/snps_norm_filter.vcf.gz
  tabix -p vcf ${params.path_nextflow_dir}/4.SNIPPY/${base}/snps_norm_filter.vcf.gz
  """
}
...
process ANNOTATE_VCFs {
  ...
  script:
  """
  snpEff -nodownload -c ${params.path_nextflow_dir}/Files/snpEff.config -v AP018935.1 ${params.path_nextflow_dir}/4.SNIPPY/${base}/snps_norm_filter.vcf.gz | gzip -c > ${params.path_nextflow_dir}/4.SNIPPY/${base}/snps_norm_filter_ANN.vcf.gz
  """
}
...
process QUAST {
  conda "bioconda::quast conda-forge::setuptools conda-forge::joblib python=3.10"
...
process POPPUNK {
  ...
  head -1 ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt > ${params.path_nextflow_dir}/Files/phenotypes_filtered_SC.txt
...
script:
  ...
  # Also do for serotypes
  awk '(NR == 1) || (FNR > 1)' ${params.path_nextflow_dir}/3.SEROTYPE_MLST/*__genes__GBS-SBG__results.txt | sed 's/GBS-SBG://g' > ${params.path_nextflow_dir}/10.ASSIGN_CC/Serotypes.txt
  ...
  # Keep only the samples with classified CC for pyseer
  awk 'NR>1{s[\$1]=1} END{for(k in s)print k}' ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt > ${params.path_nextflow_dir}/10.ASSIGN_CC/IDs.txt
  printf "Sample\\tclonal_complex\\n" > ${params.path_nextflow_dir}/10.ASSIGN_CC/CC_clusters.txt
  awk -F',' 'NR==1{next} {sub(/^cc/,"",\$3); if(\$3!="NA") print \$2 "\\t" \$3}' ${params.path_nextflow_dir}/10.ASSIGN_CC/combined_results_with_cc.txt | awk 'NR==FNR{s[\$1]=1;next}(\$1 in s)' ${params.path_nextflow_dir}/10.ASSIGN_CC/IDs.txt - >> ${params.path_nextflow_dir}/10.ASSIGN_CC/CC_clusters.txt
  awk 'NR>1{s[\$1]=1} END{for(k in s)print k}' ${params.path_nextflow_dir}/10.ASSIGN_CC/CC_clusters.txt > ${params.path_nextflow_dir}/10.ASSIGN_CC/IDs2.txt
  head -1 ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt > ${params.path_nextflow_dir}/Files/phenotypes_filtered_CC.txt
  awk 'NR==FNR{s[\$1]=1;next}NR>1&&(\$1 in s)' ${params.path_nextflow_dir}/10.ASSIGN_CC/IDs2.txt ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt >> ${params.path_nextflow_dir}/Files/phenotypes_filtered_CC.txt
  rm ${params.path_nextflow_dir}/10.ASSIGN_CC/IDs.txt ${params.path_nextflow_dir}/10.ASSIGN_CC/IDs2.txt
  """
}
...
conda "bcftools"
  ...
  script:
  """
  bcftools merge -m none -0 -O z ${params.path_nextflow_dir}/4.SNIPPY/SRR*/snps_norm_filter.vcf.gz > ${params.path_nextflow_dir}/11.SNIPPY_MULTI/merged.vcf.gz"
  tabix -p vcf ${params.path_nextflow_dir}/11.SNIPPY_MULTI/merged.vcf.gz
  """
}

process GET_CORE_POS {
  input:
  val base
  ...
  script:
  """
    awk '/^>/ {if (seq) print name "\\n" seq; name=\$0; seq=\"\"; next} {seq=seq \$0} END{if (seq) print name "\\n" seq}' ${params.path_nextflow_dir}/11.SNIPPY_MULTI/clean.full.aln |
    grep -v '^>' |
    tail -n +2 |
    sed 's/./&\\t/g; s/\\t\$//' |
    awk '{for(i=1;i<=NF;i++) if(\$i=="-" || \$i=="N") col[i]++;n++}END {for(i=1;i<=NF;i++) printf "%d %.5f\\n", i, (col[i]?col[i]:0)/n}' > ${params.path_nextflow_dir}/11.SNIPPY_MULTI/Missingness_Prop.txt

    awk '\$2 < 0.05 {print "AP018935.1\\t" \$1}' ${params.path_nextflow_dir}/11.SNIPPY_MULTI/Missingness_Prop.txt > ${params.path_nextflow_dir}/11.SNIPPY_MULTI/Keep_Pos_max5missing.txt
  """
}


process RAxML_MLsearch {
  conda "${params.path_conda_envs}/raxml-ng"
  ...
  raxml-ng --msa ${params.path_nextflow_dir}/11.SNIPPY_MULTI/clean.full.aln --model GTR+G --threads auto{48} --seed 12345 --prefix ${params.path_nextflow_dir}/12.Phylogeny/12.1.RAxML/T1
  """
}
...
process RAxML_bootstrap {
  ...
  raxml-ng --bootstrap --msa ${params.path_nextflow_dir}/11.SNIPPY_MULTI/clean.full.aln --model GTR+G --threads auto{48}  --seed 12345 --prefix ${params.path_nextflow_dir}/12.Phylogeny/12.1.RAxML/T2
  """
}
...
process RAxML_bipartition {
 ...
 cpus 48
  ...
  raxml-ng --support --tree ${params.path_nextflow_dir}/12.Phylogeny/12.1.RAxML/T1.raxml.bestTree --bs-trees ${params.path_nextflow_dir}/12.Phylogeny/12.1.RAxML/T2.raxml.bootstraps --threads auto{48} --prefix ${params.path_nextflow_dir}/12.Phylogeny/12.1.RAxML/T3
  """
}
...
process FASTTREE {
  ...
  ${params.path_nextflow_dir}/Files/FastTree_DUSE_DOUBLE -gtr -gamma -nt ${params.path_nextflow_dir}/11.SNIPPY_MULTI/clean.full.aln > ${params.path_nextflow_dir}/12.Phylogeny/12.2.FastTree/GBS_GWAS_FastTree_phylo.tre
  """
}
...
process ROARY {
  conda "${params.path_conda_envs}/roary"
...
process FILTER_CORE_SNPs {

  input:
  val MERGE_VCF_out
  val GET_CORE_POS_out
  ...
  script:
  """
  zcat ${params.path_nextflow_dir}/11.SNIPPY_MULTI/merged.vcf.gz | awk 'BEGIN { while ((getline < "${params.path_nextflow_dir}/11.SNIPPY_MULTI/Keep_Pos_max5missing.txt") > 0) s[\\\$1 ":" \\\$2]=1 } /^#/ {print; next} (s[\\\$1 ":" \\\$2])' > ${params.path_nextflow_dir}/11.SNIPPY_MULTI/merged_max5missing.vcf
  """
}
...
process Fasttree_sim {
  ...
  python ${params.path_nextflow_dir}/Files/pyseer/scripts/phylogeny_distance.py --lmm ${params.path_nextflow_dir}/12.Phylogeny/12.2.FastTree/GBS_GWAS_FastTree_phylo.tre > ${params.path_nextflow_dir}/15.Pyseer/15.1.Main_analysis/FastTree_phylogeny_K.tsv
  """
}


process SNPGWAS_Fasttree_LMM {
  cpus 8
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95  --lmm --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt --phenotype-column binary --vcf ${params.path_nextflow_dir}/11.SNIPPY_MULTI/merged_max5missing.vcf --similarity ${params.path_nextflow_dir}/15.Pyseer/15.1.Main_analysis/FastTree_phylogeny_K.tsv --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.1.Main_analysis/SNPGWAS_Fasttree_LMM.txt
  """
}

process DBGWAS_Fasttree_LMM {
  cpus 8
...
process PanGWAS_Panaroo_Fasttree_LMM {
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95 --lmm --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt --phenotype-column binary --pres ${params.path_nextflow_dir}/13.Pangenome_analysis/13.1.Panaroo/gene_presence_absence.Rtab --similarity ${params.path_nextflow_dir}/15.Pyseer/15.1.Main_analysis/FastTree_phylogeny_K.tsv --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.1.Main_analysis/PanGWAS_Panaroo_Fasttree_LMM.txt
  """
}
...
process Fasttree_dist {
  ...
  mkdir -p ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.1.Fasttree_FEM

  # create phylogeny tsv file
  python ${params.path_nextflow_dir}/Files/pyseer/scripts/phylogeny_distance.py ${params.path_nextflow_dir}/12.Phylogeny/12.2.FastTree/GBS_GWAS_FastTree_phylo.tre > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.1.Fasttree_FEM/FastTree_phylogeny_distances.tsv
  """
}

process SNPGWAS_Fasttree_FEM {
  cpus 8
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95  --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt --phenotype-column binary --vcf ${params.path_nextflow_dir}/11.SNIPPY_MULTI/merged_max5missing.vcf --distances ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.1.Fasttree_FEM/FastTree_phylogeny_distances.tsv --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.1.Fasttree_FEM/SNPGWAS_Fasttree_FEM.txt
  """
}

process DBGWAS_Fasttree_FEM {
  cpus 8
...
process PanGWAS_Panaroo_Fasttree_FEM {
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95 --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt --phenotype-column binary --pres ${params.path_nextflow_dir}/13.Pangenome_analysis/13.1.Panaroo/gene_presence_absence.Rtab --distances ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.1.Fasttree_FEM/FastTree_phylogeny_distances.tsv --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.1.Fasttree_FEM/PanGWAS_Panaroo_Fasttree_FEM.txt
  """
}
...
process RAxML_dist {
  ...
  mkdir -p ${params.path_nextflow_dir}/15.Pyseer/
  mkdir -p ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1
  mkdir -p ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.2.RAxML_LMM

  # create phylogeny tsv file
  python ${params.path_nextflow_dir}/Files/pyseer/scripts/phylogeny_distance.py --lmm ${params.path_nextflow_dir}/12.Phylogeny/12.1.RAxML/T1.raxml.bestTree > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.2.RAxML_LMM/RAxML_phylogeny_K.tsv
  """
}
...
process SNPGWAS_RAxML {
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95 --lmm --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt --phenotype-column binary --vcf ${params.path_nextflow_dir}/11.SNIPPY_MULTI/merged_max5missing.vcf --similarity ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.2.RAxML_LMM/RAxML_phylogeny_K.tsv --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.2.RAxML_LMM/SNPGWAS_RAxML.txt
  """
}
...
process PanGWAS_Panaroo_RAxML {
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95 --lmm --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt --phenotype-column binary --pres ${params.path_nextflow_dir}/13.Pangenome_analysis/13.1.Panaroo/gene_presence_absence.Rtab --similarity ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.2.RAxML_LMM/RAxML_phylogeny_K.tsv --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.2.RAxML_LMM/PanGWAS_Panaroo_RAxML.txt
  """
}
...
process Mash_dist {
  ...
  mkdir -p ${params.path_nextflow_dir}/15.Pyseer
  mkdir -p ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1
  mkdir -p ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.3.Mash

  # create distance file
  mash sketch -s 10000 -o ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.3.Mash/mash_sketch ${params.path_nextflow_dir}/5.ASSEMBLIES/*.fasta
  mash dist ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.3.Mash/mash_sketch.msh ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.3.Mash/mash_sketch.msh  | square_mash > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.3.Mash/mash.tsv
  """
}
...
process SNPGWAS_Mash {
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95 --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt --phenotype-column binary --vcf ${params.path_nextflow_dir}/11.SNIPPY_MULTI/merged_max5missing.vcf --distances ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.3.Mash/mash.tsv --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.3.Mash/SNPGWAS_mash.txt
  """
}
...
process PanGWAS_Panaroo_Mash {
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95 --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt --phenotype-column binary --pres ${params.path_nextflow_dir}/13.Pangenome_analysis/13.1.Panaroo/gene_presence_absence.Rtab --distances ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.3.Mash/mash.tsv --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.3.Mash/PanGWAS_Panaroo_mash.txt
  """
}
...
process SC_dist {
  ...
  mkdir -p ${params.path_nextflow_dir}/15.Pyseer
  mkdir -p ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1
  mkdir -p ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.4.SC

  # create distances file
  sed 's/,/\t/g' ${params.path_nextflow_dir}/9.POPPUNK/GBS_GWAS_db/GBS_GWAS_db_core_clusters.csv > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.4.SC/SC_clusters.txt
  """
}
...
process SNPGWAS_SC {
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95 --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered_SC.txt --phenotype-column binary --vcf ${params.path_nextflow_dir}/11.SNIPPY_MULTI/merged_max5missing.vcf --covariates ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.4.SC/SC_clusters.txt --use-covariates 2 --no-distances --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.4.SC/SNPGWAS_SC.txt
  """
}
...
process PanGWAS_Panaroo_SC {
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95 --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered_SC.txt --phenotype-column binary --pres ${params.path_nextflow_dir}/13.Pangenome_analysis/13.1.Panaroo/gene_presence_absence.Rtab --covariates ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.4.SC/SC_clusters.txt --use-covariates 2 --no-distances --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.4.SC/PanGWAS_Panaroo_SC.txt
  """
}
...
process CC_dist {
  ...
  mkdir -p ${params.path_nextflow_dir}/15.Pyseer
  mkdir -p ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1
  mkdir -p ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.5.CC

  # create distances file
  cp ${params.path_nextflow_dir}/10.ASSIGN_CC/CC_clusters.txt > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.5.CC/CC_clusters.txt
  """
}
...
process SNPGWAS_CC {
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95 --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered_CC.txt --phenotype-column binary --vcf ${params.path_nextflow_dir}/11.SNIPPY_MULTI/merged_max5missing.vcf --covariates ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.5.CC/CC_clusters.txt --use-covariates 2 --no-distances --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.5.CC/SNPGWAS_CC.txt
  """
}
...
process DBGWAS_CC {
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95 --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered_CC.txt --phenotype-column binary --kmers ${params.path_nextflow_dir}/14.UNITIGS/unitigs.txt --uncompressed --output-patterns ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.5.CC/DBGWAS_CC_unitig_patterns.txt --covariates ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.5.CC/CC_clusters.txt --use-covariates 2 --no-distances --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.5.CC/DBGWAS_CC.txt
  annotate_hits_pyseer ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.5.CC/DBGWAS_CC.txt ${params.path_nextflow_dir}/Files/DBGWAS_references.txt ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.5.CC/DBGWAS_CC_annotation.txt
  """
}

process PanGWAS_Panaroo_CC {
  cpus 8
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95 --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered_CC.txt --phenotype-column binary --pres ${params.path_nextflow_dir}/13.Pangenome_analysis/13.1.Panaroo/gene_presence_absence.Rtab --covariates ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.5.CC/CC_clusters.txt --use-covariates 2 --no-distances --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.5.CC/PanGWAS_Panaroo_CC.txt
  """
}
...
process PanGWAS_Roary_Fasttree {
  ...
  # Run Pyseer
  pyseer --min-af 0.05 --max-af 0.95 --lmm --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt --phenotype-column binary --pres ${params.path_nextflow_dir}/13.Pangenome_analysis/13.2.ROARY/gene_presence_absence.Rtab --similarity ${params.path_nextflow_dir}/15.Pyseer/15.1.Main_analysis/FastTree_phylogeny_K.tsv --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.3.Subanalysis_2/PanGWAS_Panaroo_Fasttree_LMM.txt
...
process PanGWAS_Panaroo_CLARC_Fasttree {
  ...
  # Run Pyseer
  pyseer --min-af 0.05 --max-af 0.95 --lmm --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt --phenotype-column binary --pres ${params.path_nextflow_dir}/13.Pangenome_analysis/13.3.Panaroo_CLARC/Output/clarc_results/clarc_condensed_presence_absence.Rtab --similarity ${params.path_nextflow_dir}/15.Pyseer/15.1.Main_analysis/FastTree_phylogeny_K.tsv --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.3.Subanalysis_2/PanGWAS_Panaroo_CLARC_Fasttree.txt
  """
}
...
process PanGWAS_Roary_CLARC_Fasttree {
  ...
  # Run Pyseer
  pyseer --min-af 0.05 --max-af 0.95 --lmm --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt --phenotype-column binary --pres ${params.path_nextflow_dir}/13.Pangenome_analysis/13.4.ROARY_CLARC/Output/clarc_results/clarc_condensed_presence_absence.Rtab --similarity ${params.path_nextflow_dir}/15.Pyseer/15.1.Main_analysis/FastTree_phylogeny_K.tsv --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.3.Subanalysis_2/PanGWAS_Roary_CLARC_Fasttree.txt
  """
}
...
process SNPGWAS_continuous_Fasttree {
  ...
  # Run Pyseer
  pyseer --min-af 0.05 --max-af 0.95 --lmm --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt --continuous --phenotype-column continuous --vcf ${params.path_nextflow_dir}/11.SNIPPY_MULTI/merged_max5missing.vcf --similarity ${params.path_nextflow_dir}/15.Pyseer/15.1.Main_analysis/FastTree_phylogeny_K.tsv --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.4.Subanalysis_3/SNPGWAS_continuous_Fasttree.txt
  """
}
...
main:
    ...
    // Step 5: FILTER_VCFs depend on SNIPPY's output
    FILTER_VCFs(SNIPPY.out)

    // Step 5: ANNOTATE_VCFs depend on FILTER_VCFs's output
    ANNOTATE_VCFs(FILTER_VCFs.out)
...
workflow flow3 {
    ...
    // Step 4: FILTER_CORE_SNPs depends on MERGE_VCF and GET_CORE_POS outputs
    FILTER_CORE_SNPs(MERGE_VCF.out[0], GET_CORE_POS.out[0])
...
workflow flow_main {
    ...
    main:
    Fasttree_sim(fasttree_data, unitig_data, panaroo_data)
    SNPGWAS_Fasttree_LMM(Fasttree_sim.out[0])
    DBGWAS_Fasttree_LMM(Fasttree_sim.out[0])
    PanGWAS_Panaroo_Fasttree_LMM(Fasttree_sim.out[0])
}
...
workflow flow_sub1_Fasttree {
    ...
    SNPGWAS_Fasttree_FEM(Fasttree_dist.out[0])
    DBGWAS_Fasttree_FEM(Fasttree_dist.out[0])
    PanGWAS_Panaroo_Fasttree_FEM(Fasttree_dist.out[0])
}

workflow flow_sub1_RAxML {
    take:
    raxml_data
    unitig_data
    panaroo_data
...
workflow flow_sub1_CC {
    take:
    unitig_data
    panaroo_data

    main:
    CC_dist(unitig_data, panaroo_data)
    SNPGWAS_CC(CC_dist.out[0])
    DBGWAS_CC(CC_dist.out[0])
    PanGWAS_Panaroo_CC(CC_dist.out[0])
}
...
workflow flow_sub2 {
    ...
    main:
    Fasttree_sim(fasttree_data, panaroo_clarc_data, roary_clarc_data)
    PanGWAS_Roary_Fasttree(Fasttree_sim.out[0])
    PanGWAS_Panaroo_CLARC_Fasttree(Fasttree_sim.out[0])
    PanGWAS_Roary_CLARC_Fasttree(Fasttree_sim.out[0])
}
...
workflow flow_sub3 {
    ...
    main:
    Fasttree_sim(fasttree_data, unitig_data, panaroo_data)
    SNPGWAS_continuous_Fasttree(Fasttree_sim.out[0])
    DBGWAS_continuous_Fasttree(Fasttree_sim.out[0])
    PanGWAS_continuous_Fasttree(Fasttree_sim.out[0])
}
...
flow2(flow1.out)
flow3(flow2.out.collect())


  if( params.main ) {
    flow_main(flow3.out[0], flow3.out[1], flow3.out[2])
    println "Running the Main Analysis: SNP-GWAS, DB-GWAS and Pan-GWAS, with the population structure controlled using a Fastree phylogeny and the accessory genome defined per Panaroo."
  } else {
    println "Skipping the Main Analysis (SNP-GWAS, DB-GWAS and Pan-GWAS, with the population structure controlled using a Fastree phylogeny and the accessory genome defined per Panaroo)."
  }
  if( params.sub1 ) {
    flow_sub1_Fasttree(flow3.out[0], flow3.out[1], flow3.out[2])
    flow_sub1_RAxML(flow3.out[5], flow3.out[1], flow3.out[2])
    flow_sub1_Mash(flow3.out[3], flow3.out[1], flow3.out[2])
    flow_sub1_SC(flow3.out[4], flow3.out[1], flow3.out[2])
    flow_sub1_CC(flow3.out[1], flow3.out[2])
    println "Running the Sub-Analysis #1: SNP-GWAS, DB-GWAS and Pan-GWAS, with the population structure controlled using a RAxML phylogeny, Mash distances, Sequence Clusters (SC) and Clonal Complex (CC)."
  }
  if( params.RAxML ) {
    flow_sub1_RAxML(flow3.out[5], flow3.out[1], flow3.out[2])
    println "Running SNP-GWAS, DB-GWAS and Pan-GWAS, with the population structure controlled using a RAxML phylogeny and the accessory genome defined per Panaroo.."
  }
  if( params.Mash ) {
    flow_sub1_Mash(flow3.out[3],flow3.out[1], flow3.out[2])
  ...
  if( params.CC ) {
  flow_sub1_CC(flow3.out[1], flow3.out[2])
    println "Running SNP-GWAS, DB-GWAS and Pan-GWAS, with the population structure controlled using Clonal Complex (CC) classifications and the accessory genome defined per Panaroo.."
  }
  if( params.sub2 ) {
