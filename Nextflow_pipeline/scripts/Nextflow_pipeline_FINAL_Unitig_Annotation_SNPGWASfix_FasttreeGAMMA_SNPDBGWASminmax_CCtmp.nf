...
script:
  ...
  awk -F',' 'NR==1 {print "Taxon\tclonal_complex"; next} {sub(/^cc/, "", $3); if ($3 != "NA") print $2 "\t" $3 }' ${params.path_nextflow_dir}/10.ASSIGN_CC/combined_results_with_cc.txt > ${params.path_nextflow_dir}/10.ASSIGN_CC/CC_clusters.txt
  head -1 ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt > ${params.path_nextflow_dir}/Files/phenotypes_filtered_CC.txt
  awk 'NR==FNR { if (NR>1) s[$1]=1; next } NR==1 || $1 in s' ${params.path_nextflow_dir}/10.ASSIGN_CC/CC_clusters.txt ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt >> ${params.path_nextflow_dir}/Files/phenotypes_filtered_CC.txt
  """
}
...
process GET_CORE_POS {
  ...
  script:
  """
  awk '\''/^>/ {if (seq) print name \"\n\" seq; name=\$0; seq=\"\"; next} {seq=seq \$0} END{if (seq) print name \"\n\" seq}'\'' ${params.path_nextflow_dir}/11.SNIPPY_MULTI/clean.full.aln | grep -v '\''^>'\'' | tail -n +2 | sed '\''s/./&\t/g; s/\t$//'\'' | awk '\''{for(i=1;i<=NF;i++) if(\$i==\"-\" || \$i==\"N\") col[i]++;n++}END {for(i=1;i<=NF;i++) printf \"%d %.5f\n\", i, (col[i]?col[i]:0)/n}'\'' > ${params.path_nextflow_dir}/11.SNIPPY_MULTI/Missingness_Prop.txt
...
process FILTER_CORE_SNPs {
  ...
  script:
  """
  bcftools view -R ${params.path_nextflow_dir}/11.SNIPPY_MULTI/Keep_Pos_max5missing.txt ${params.path_nextflow_dir}/11.SNIPPY_MULTI/merged.vcf.gz > ${params.path_nextflow_dir}/11.SNIPPY_MULTI/merged_max5missing.vcf
  """
}
...
process SNPGWAS_Fasttree_FEM {
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95  --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt --phenotype-column binary --vcf ${params.path_nextflow_dir}/11.SNIPPY_MULTI/core.vcf --distances ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.1.Fasttree_FEM/FastTree_phylogeny_distances.tsv --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.1.Fasttree_FEM/SNPGWAS_Fasttree_FEM.txt
  """
}
...
process SNPGWAS_RAxML {
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95 --lmm --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt --phenotype-column binary --vcf ${params.path_nextflow_dir}/11.SNIPPY_MULTI/core.vcf --similarity ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.2.RAxML_LMM/RAxML_phylogeny_K.tsv --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.2.RAxML_LMM/SNPGWAS_RAxML.txt
  """
}
...
process SNPGWAS_Mash {
  ...
  script:
  """
  pyseer --min-af 0.05 --max-af 0.95 --phenotypes ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt --phenotype-column binary --vcf ${params.path_nextflow_dir}/11.SNIPPY_MULTI/core.vcf --distances ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.3.Mash/mash.tsv --cpu 8 > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.3.Mash/SNPGWAS_mash.txt
  """
}
...
process CC_dist {
  ...
  # create distances file
  awk 'NR==FNR { if (NR>1) s[$1]=1; next } NR==1 || $1 in s' ${params.path_nextflow_dir}/Files/phenotypes_filtered.txt ${params.path_nextflow_dir}/10.ASSIGN_CC/CC_clusters.txt > ${params.path_nextflow_dir}/15.Pyseer/15.2.Subanalysis_1/15.2.5.CC/CC_clusters.txt
  """
}
